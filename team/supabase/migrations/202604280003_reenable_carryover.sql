-- Re-enable carryover reconstruction in admin dashboard.
-- The previous migration (202604280002) disabled it with a degraded path that
-- returned '[]' for carryoverAging and completedTasks to avoid query timeouts.
-- This migration restores the full implementation from sql/cutover/05-dashboard-attendance.sql
-- with a 365-day lookback cap on historical submissions.
...
begin
  ...
  -- Date-ranged submissions for compliance metrics
  sod_latest as (
  ...
  -- Historical submissions for carryover reconstruction (365-day lookback cap)
  sod_all_latest as (
    select distinct on (e.department, e.employee_name, s.work_date)
      e.department, e.employee_name, s.work_date, s.submitted_at, s.task_count, s.payload_json
    from emp e
  ...
  -- Explode SOD task lists from historical submissions
  sod_tasks_all as (
    select
      s.department,
      s.employee_name,
  ...
  -- Explode EOD update lists from historical submissions
  ...
  -- Pending tasks carried forward in SOD payloads (not yet 100% complete)
  ...
  -- Infer task state for days where SOD was submitted but EOD was not
  ...
  -- Working day calendar derived from actual submission activity
  ...
  -- Open carryover tasks (from SOD task lists, not yet completed in EOD)
  open_carry_rows as (
    select
      o.department,
      o.employee_name,
  ...
  -- Admin remove/rename markers for carryover tasks
  removed_markers as (
    select distinct on (
      public.canonical_department_key(e.department),
      lower(trim(e.employee_name)),
  ...
  -- Completed tasks auto-detected from EOD updates crossing 100% within date range
  ...
  -- Completed tasks marked via admin "Complete" action
  ...
  -- Open carryover count per employee (for KPI/compliance rows)
  open_carry as (
    select department, employee_name, count(*)::int as open_carryover
    from open_carry_rows_all
    group by department, employee_name
  ),
  -- Rate-limited result sets for the dashboard
  ...
  -- Compliance metrics (date-range scoped)
